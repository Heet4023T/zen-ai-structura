...
def write_business_layout(ws, data):
    ...
    if has_disc:
        headers.extend(["Gross", "Disc"]); keys.extend(["gross_amount", "discount_amount"]); widths.extend([12, 10])
...
def write_personal_layout(ws, data):
    ...
    widths = [40, 10, 15, 15]
    for i, (h, w) in enumerate(zip(headers, widths), 1):
        c = ws.cell(row=3, column=i, value=h); c.font = Font(bold=True); c.border = Border(bottom=Side(style='thin'))
        ws.column_dimensions[get_column_letter(i)].width = w
    curr = 4
    for item in items:
